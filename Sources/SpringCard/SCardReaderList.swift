 ...
 */
@objcMembers
open class SCardReaderList: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    /// :nodoc:
    @objc public init(device: CBPeripheral, centralManager: CBCentralManager, delegate: SCardReaderListDelegate?, secureConnectionParameters: SecureConnectionParameters?) {
        #if DEBUG
...
//        self.device.delegate = self
    ...
    public subscript(slot: Int) -> SCardReader? {
        return self.getReaderFromIndex(slot: slot)
    }
    ...
    public subscript(slot: String) -> SCardReader? {
        return self.getReaderFromString(slot: slot)
    }
    ...
    private func generateErrorAfterReading(characteristic: CBCharacteristic, errorReceived: Error?) {
        ...
        if characteristic.uuid == self.CCID_Status_Characteristic?.uuid {
            if self.machineState != .discoveredDeviceWithSuccess {
                ...
                delegate?.onReaderStatus(reader: nil, present: false, powered: false, error: _error)
            }
    ...
    /// :nodoc:
    @objc public func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        #if DEBUG
    ...
    /**
     ...
     - Returns: a `Reader` object or nil if the slot's name is unknown
     */
    @objc public func getReaderFromString(slot: String) -> SCardReader? {
        #if DEBUG
    ...
    /**
     ...
     - Returns: a `Reader` object or nil if the index is out of bounds
     */
    @objc public func getReaderFromIndex(slot: Int) -> SCardReader? {
        #if DEBUG
        os_log("SCardReaderList:getReader()", log: OSLog.libLog, type: .info)
    ...
    public func wakeUp() {
        ...
        guard let CCID_Status_Characteristic = self.CCID_Status_Characteristic else {
            ...
            self.delegate?.onReaderStatus(reader: nil, present: false, powered: false, error: error)
            return
        }
    ...
    private func callOnPowerInfo(_ powerState: Int?, _ batteryLevel: Int?, _ error: Error?) {
        ...
        #endif
        self.delegate?.onPowerInfo(powerState: powerState ?? -1, batteryLevel: batteryLevel ?? -1, error: error)
    }
